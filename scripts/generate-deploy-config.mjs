...
${indent(yamlList(serviceNames))}
      ...
      emergency_api_bootstrap:
        type: boolean
        description: 'Break-glass API production bootstrap when the deployed authorizer blocks its own fix'
        required: false
        default: false
      ...
      - name: Authorize exact deployment operation
        ...
        run: |
          ...
          if [ "$INPUT_EMERGENCY_API_BOOTSTRAP" = true ]; then
            ...
            trains_file="$(mktemp)"
            trap 'rm -f "$controls_file" "$trains_file"' EXIT
            controls_status="$(curl --silent --show-error \
              --connect-timeout 10 \
              --max-time 60 \
              --output "$controls_file" \
              --write-out '%{http_code}' \
              "$RELEASE_BUS_API_URL/deploy/release-bus-v2/controls")"
            test "$controls_status" = 200
            jq -e '
              .mode == "PRODUCTION" and
              ([.controls[] | select(.scope == "ALL" and .paused == false)] | length == 1) and
              ([.lanes[] | select(.lane == "PRODUCTION" and .status == "OFF" and .changeable == true)] | length == 1) and
              ([.locks[] | select(
            ...
            trains_status="$(curl --silent --show-error \
              --connect-timeout 10 \
              --max-time 60 \
              --output "$trains_file" \
              --write-out '%{http_code}' \
              "$RELEASE_BUS_API_URL/deploy/release-bus-v2/trains")"
            test "$trains_status" = 200
            jq -e '
              [.trains[] | select(
                .lane == "PRODUCTION" and
            ...
            jq -cn \
              ...
              '{emergency_api_bootstrap:true,actor:$actor,sha:$sha,reason:$reason}'
            exit 0
          fi
