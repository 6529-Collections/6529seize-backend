    ...
    steps:
      ...
      - name: Select environment-specific config
        ...
        run: |
          ...
          # AVIF conversion always uses private ingest, even when general image
          # sanitization is disabled. The shared bucket is owned by staging.
          if [ -z "$DROP_MEDIA_INGEST_BUCKET" ]; then
            DROP_MEDIA_ACCOUNT_ID="$(aws sts get-caller-identity --query Account --output text)"
            DROP_MEDIA_INGEST_BUCKET="6529-drop-media-ingest-$DROP_MEDIA_ACCOUNT_ID"
          fi
      ...
      - name: Deploy API
        ...
        run: |
          ...
          jq --arg commit "$GIT_COMMIT" --arg claimsMediaArweaveUploadSqsUrl "$CLAIMS_MEDIA_ARWEAVE_UPLOAD_SQS_URL" --arg attachmentsIngestS3Bucket "$ATTACHMENTS_INGEST_S3_BUCKET" --arg dropMediaSanitizeImages "$DROP_MEDIA_SANITIZE_IMAGES" --arg dropMediaIngestS3Bucket "$DROP_MEDIA_INGEST_S3_BUCKET" --arg dropMediaIngestS3Region "$DROP_MEDIA_INGEST_S3_REGION" --arg dropMediaIngestStage "$DROP_MEDIA_INGEST_STAGE" --arg dropMediaSanitizerSqsQueueName "$DROP_MEDIA_SANITIZER_SQS_QUEUE_NAME" --arg apiGatewayWsEndpoint "$API_GATEWAY_WS_ENDPOINT" 'del(.MEMBERSHIP_SOURCE_TRACKING_MODE, .MEMBERSHIP_SOURCE_TRACKING_STAGE, .MEMBERSHIP_READ_MODE, .MEMBERSHIP_SHADOW_MODE, .MEMBERSHIP_READER_PROFILE_IDS, .MEMBERSHIP_READER_COVERAGE_REVISION, .MEMBERSHIP_READER_STAGE) + {GIT_COMMIT: $commit, CLAIMS_MEDIA_ARWEAVE_UPLOAD_SQS_URL: $claimsMediaArweaveUploadSqsUrl, ATTACHMENTS_INGEST_S3_BUCKET: $attachmentsIngestS3Bucket, DROP_MEDIA_SANITIZE_IMAGES: $dropMediaSanitizeImages, DROP_MEDIA_INGEST_S3_BUCKET: $dropMediaIngestS3Bucket, DROP_MEDIA_INGEST_S3_REGION: $dropMediaIngestS3Region, DROP_MEDIA_INGEST_STAGE: $dropMediaIngestStage, DROP_MEDIA_SANITIZER_SQS_QUEUE_NAME: $dropMediaSanitizerSqsQueueName, API_GATEWAY_WS_ENDPOINT: $apiGatewayWsEndpoint}' /tmp/current_env.json > /tmp/app_env.json
