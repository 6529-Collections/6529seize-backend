...
const sourcePath = path.join(repoRoot, 'config', 'deploy-services.json');
...
function buildGeneratedTs(config) {
  const serviceConfigs = JSON.stringify(config.services, null, 2);
  return `// This file is generated by scripts/generate-deploy-config.mjs.
// Edit config/deploy-services.json and rerun the generator instead.

export const GENERATED_DEPLOY_DEFAULT_ENVIRONMENT = ${JSON.stringify(
    config.default_environment
  )} as const;

export const GENERATED_DEPLOY_DEFAULT_SERVICE = ${JSON.stringify(
    config.default_service
  )} as const;

export const GENERATED_DEPLOY_SERVICE_CONFIGS = ${serviceConfigs} as const;
`;
}
...
async function main() {
  ...
  await fs.writeFile(generatedTsPath, buildGeneratedTs(config));
}
