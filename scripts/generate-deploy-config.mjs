      ...
      - name: Validate dispatch inputs before using credentials
        ...
        run: |
          ...
          if [ "$source_tracking_mode" = tracking-v1 ]; then
            if [ "$INPUT_ENVIRONMENT" != staging ] || ! [[ "$INPUT_SERVICE" =~ ^(api|overRatesRevocationLoop|tdhLoop|xTdhLoop|xTdhGrantsReviewerLoop|delegationsLoop|nftOwnersLoop|externalCollectionSnapshottingLoop|externalCollectionLiveTailingLoop|helpBotReplyLoop|populateHistoricConsolidatedTdh)$ ]]; then
