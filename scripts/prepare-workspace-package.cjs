...
if (!targetArg) {
  console.error(
    'Usage: node scripts/prepare-workspace-package.cjs <target-dir> [--prod]'
  );
  process.exit(1);
}
...
if (!fs.existsSync(packageJsonPath)) {
  console.error(
    'prepare-workspace-package.cjs must run from a workspace package directory.'
  );
  process.exit(1);
}
...
if (
  ...
  console.error(
    'Workspace package directory must be inside the repository root.'
  );
  process.exit(1);
}
