...
exports.up = async function(db) {
  var anchors = await db.runSql(
    `select wd.wave_id, wd.drop_id, mc.claim_id
  ...
  if (!anchors.length) {
    throw new Error('Cannot backfill Main Stage Meme cards without minting claim anchors');
  }

  var waveIds = Array.from(new Set(anchors.map(function(anchor) {
    return anchor.wave_id;
  })));
  if (waveIds.length !== 1) {
    throw new Error(
      'Expected minting claim anchors in exactly one wave, found: ' +
        waveIds.join(', ')
    );
  }
  var mainStageWaveId = waveIds[0];
