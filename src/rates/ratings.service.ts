  ...
  async bulkRep({ targets }: BulkRepRequest, ctx: RequestContext) {
    ...
    await this.identitiesDb.executeNativeQueriesInTransaction(
      async (connection) => {
        ...
        const ratingChanges: {
          profileId: string;
          changes: { category: string; oldRating: number; newRating: number }[];
        }[] = Object.entries(
          await this.ratingsDb
            .getAllRepRatingsForTargetsAndCategories(
              {
            ...
            .then((existingRatings) =>
              Object.entries(newRatingsByCategoryAndProfile).reduce(
        ...
        const logs = ratingChanges
          .map<ProfileActivityLog[]>((profileChange) =>
            profileChange.changes.map<ProfileActivityLog>((ratingChange) => ({
              ...
              proxy_id: authenticationContext.getLoggedInUsersProfileId()
            }))
          )
          .flat();
