...
function validateRoutes(
  ...
  cmsPackage.payload.routes.forEach((route, index) => {
    ...
    if (route.kind === 'page') {
      validatePageRoute(route, index, pageMap, issues);
    } else {
      validateNonPageRoute(
        route,
        index,
        cmsPackage.profile.handle,
        routePathSet,
...
function validatePageRoute(
  route: CmsPackageV1['payload']['routes'][number],
  index: number,
  pageMap: IdMap<CmsPageV1>,
  issues: CmsValidationIssueV1[]
): void {
...
function isSafeRelativeUri(value: string): boolean {
  ...
  if (hasUnsafeRelativeUriCharacter(value)) {
    return false;
  }
