...
export class ModerationReviewDb extends LazyDbAccessCompatibleService {
  ...
  async groupDefinition(
    ...
    const members = groupIds.length
      ? await this.db.execute<{ profile_group_id: string; profile_id: string }>(
  ...
  async reportForReview(id: string, ctx: RequestContext = {}) {
    const report = await this.db.oneOrNull<{
      id: string;
      item_id: string | null;
      drop_id: string;
      ...
      `select id,item_id,drop_id,author_profile_id,reporter_profile_id,reason,content_snapshot,ai_recommendation,ai_category from ${CONTENT_MODERATION_REPORTS_TABLE} where id=:id`,
      { id },
      this.options(ctx)
    );
  ...
  async retain(ctx: RequestContext = {}) {
    ...
    await this.db.execute(
      `update ${ITEMS} i set i.review_status='REVIEWED',i.evidence_expires_at=coalesce(i.evidence_expires_at,:resolvedExpiry)
      ...
      { resolvedExpiry: Date.now() + 90 * 86400000 },
      this.options(ctx)
    );
