...
import { AD_WORKS, AD_CONTEXTS, AD_ARTISTS, AD_ARTIST_REVISIONS, AD_SOURCES, AD_DROP_LINKS, AD_REVISIONS, AD_GRANTS, AD_REVIEWS, AD_THREADS, AD_EVENTS, AD_IDEMPOTENCY } from '@/artwork-documentation/artwork-documentation.tables';
...
@Index('ad_context_program_lifecycle', ['program_id', 'lifecycle', 'updated_at'])
...
export class ArtworkDocumentationContextEntity extends DocumentationRow {
  ...
  @Column({ type: 'varchar', length: 100 }) owner_profile_id!: string;
  @Column({ type: 'varchar', length: 100, nullable: true }) program_id!: string | null;
  @Column({ type: 'json' }) profile_json!: string;
  @Column({ type: 'int' }) draft_version!: number;
  @Column({ type: 'varchar', length: 36, nullable: true }) artist_record_revision_id!: string | null;
  @Column({ type: 'varchar', length: 36, nullable: true }) latest_revision_id!: string | null;
...
@Index('ad_artist_revision_version', ['owner_profile_id', 'record_version'], { unique: true })
...
@Entity(AD_REVISIONS)
@Index('ad_revision_number', ['context_id', 'revision_number'], { unique: true })
@Index('ad_revision_draft', ['context_id', 'source_draft_version'], { unique: true })
...
@Entity(AD_GRANTS)
@Index('ad_grant_subject_scope', ['subject_profile_id', 'context_id', 'program_id'])
export class ArtworkDocumentationGrantEntity extends DocumentationRow {
  @Column({ type: 'varchar', length: 36, nullable: true }) context_id!: string | null;
  @Column({ type: 'varchar', length: 100, nullable: true }) program_id!: string | null;
...
export class ArtworkDocumentationReviewEntity {
  ...
  @Column({ type: 'varchar', length: 100, nullable: true }) reviewer_profile_id!: string | null;
...
export class ArtworkDocumentationThreadEntity extends DocumentationRow {
  ...
  @Column({ type: 'varchar', length: 100, nullable: true }) field_path!: string | null;
  @Column({ type: 'varchar', length: 36, nullable: true }) revision_id!: string | null;
...
export class ArtworkDocumentationIdempotencyEntity {
  ...
  @Column({ type: 'json', nullable: true }) result_reference_json!: string | null;
