import { GRADIENT_CONTRACT, MEMES_CONTRACT, NULL_ADDRESS } from './constants';
import { NFT } from './entities/INFT';
import { ConsolidatedOwnerTags, Owner } from './entities/IOwner';
...
export const findOwnerBalances = async (reset?: boolean) => {
  ...
  logger.info({
    owner_balances: startingBalances.length,
    owner_balances_memes: startingBalancesMemes.length,
    owners: startingOwners.length,
    unique_owners: uniqueOwnerWallets.size
  });
  ...
  logger.info({
    owner_balances_delta: ownersBalancesDelta.length,
    owner_balances_memes_delta: ownersBalancesMemesDelta.length
  });
  if (
    ownersBalancesDelta.some((o) =>
      areEqualAddresses(o.wallet, '0x949068b016f55ae5b47aa2cea0e082e8ee6eb1bc')
    )
  ) {
...
export async function consolidateOwnerBalances() {
  ...
  ownersBalances.forEach((ob) => {
    ...
    const consolidation = walletConsolidations.find((wc) =>
      wc.wallets.some((w: string) => areEqualAddresses(w, ob.wallet))
    );
    ...
    if (!consolidation) {
      ...
      const consolidationBalances = ownersBalances.filter((oob) =>
        consolidation.wallets.some((w: string) =>
          areEqualAddresses(w, oob.wallet)
        )
      );
  ...
  ownersBalancesMemes.forEach((obm) => {
    ...
    const consolidation = walletConsolidations.find((wc) =>
      wc.wallets.some((w: string) => areEqualAddresses(w, obm.wallet))
    );
    ...
    if (!consolidation) {
      ...
      const consolidationBalances = ownersBalancesMemes.filter((oobm) =>
        consolidation.wallets.some(
          (w: string) =>
            areEqualAddresses(w, oobm.wallet) && obm.season === oobm.season
        )
      );
      if (consolidationBalances.length > 0) {
        const totals = consolidationBalances.reduce(
