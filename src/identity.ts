...
export async function syncIdentitiesMetrics(
  ...
  await db.execute(
    `update ${IDENTITIES_TABLE} set xtdh = 0, tdh = 0, produced_xtdh = 0, granted_xtdh = 0, total_tdh = 0`,
    undefined,
    { wrappedConnection: connection }
  );
  await db.execute(
    `
     update ${IDENTITIES_TABLE} i
        ...
        set i.tdh = floor(c.total_tdh), i.total_tdh = c.total_tdh, i.xtdh = c.xtdh, i.produced_xtdh = c.produced_xtdh,  i.granted_xtdh = c.granted_xtdh 
