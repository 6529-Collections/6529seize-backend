...
const array = (
  ...
  maxItems = 30,
  minItems = 0
): ValueSchema => ({ type: 'array', items, maxItems, minItems });
const object = (
...
const number = (maximum = 1000000): ValueSchema => ({
  type: 'integer',
  minimum: 1,
  maximum
});
...
const date = object(
  {
    ...
    start: text(10, 'partial-date'),
    end: text(10, 'partial-date'),
    approximate: bool
  },
  ['precision', 'start', 'approximate']
);
const kindDetail = (kinds: string[], limit: number): ValueSchema =>
  object({ kind: choice(...kinds), detail: text(limit) }, ['kind']);
const localized = (limit: number): ValueSchema =>
  object({
...
const availability = (kinds: string[]): ValueSchema =>
  object({ kind: choice(...kinds), explanation: text(1000) }, ['kind']);
...
const schemas: Record<ModuleId, Record<string, ValueSchema>> = {
  identity: {
    ...
    preferred_credit: text(300),
    biography: localized(4000),
    links: array(object({ label: text(100), url }), 10),
    languages: array(language, 10, 1),
    private_contact: text(320),
    record_language: language
  },
  artwork: {
    title: text(255),
    title_language: language,
    alternate_titles: array(object({ language, text: text(255) }), 10),
    capture_date: date,
    ...
    edition_statement: text(500),
    visual_description: text(1500),
    series_title: text(255),
  ...
  context: {
    caption: localized(3000),
    artist_statement: localized(12000),
    making_context: text(6000),
    theme_connection: {
      oneOf: [
        object({ kind: choice('text'), text: text(4000) }),
        object({ kind: choice('caption_reference') })
      ]
    },
    misunderstandings: text(4000),
  ...
  process: {
    capture_method: kindDetail(
      ['digital_camera', 'phone', 'drone', 'film_scan', 'other'],
      500
    ),
    camera: text(300),
    lens: text(300),
    exposure_note: text(500),
    techniques: object(
      {
        kinds: array(
          choice(
    ...
    editing_tools: array(
      object({ name: text(160), version: text(100) }, ['name']),
      20
    ),
    process_description: text(8000),
    material_changes: text(4000),
    ai_use: kindDetail(['none', 'assistive', 'generative', 'unknown'], 4000),
    ingredients: object(
      {
          ...
          object(
            {
              ...
              creator: text(160),
              role: text(300),
              rights_note: text(1000)
            },
            ['creator', 'role', 'rights_note']
          ),
          50
        ),
        explanation: text(1000)
        ...
        object(
          {
            ...
            profile_id: text(100),
            role: text(300),
            authorship_claim: bool,
            credit: text(500)
          },
          ['name', 'role', 'authorship_claim', 'credit']
        ),
        30
      )
    }),
    construction_note: text(4000)
  },
  rights: {
    rights_basis: kindDetail(
      ['artist_owned', 'coauthored', 'licensed_components', 'other', 'unknown'],
      2000
    ),
    intended_license: object({ uri: url, label: text(160) }),
    rights_declaration: text(6000),
    declaration_effect: choice(
      'proposed',
      'conditional',
      'already_effective',
      'unknown'
    ),
    third_party_material: object(
      {
    ...
    consent_asset_ids: array(uuid, 30, 1),
    identifiability_note: text(2000),
    sensitive_context_note: text(4000),
    publication_notes: text(2000)
  },
  preservation: {
    significant_properties: text(6000),
    display_orientation_crop: text(2000),
    color_and_tone: text(2000),
    screen_preferences: text(2000),
    print_preferences: text(3000),
    acceptable_changes: text(4000),
    avoid_changes: text(4000),
    physical_materials: text(2000)
  },
  interview: {
    mode: choice('written', 'recording', 'declined', 'not_yet'),
    instrument_id: choice('artwork-documentation-artist-interview-v1'),
    instrument_version: { type: 'integer', enum: [1] },
    date,
    ...
    recording_permission: choice(
      'not_requested',
      'private_review',
      'intended_public_record'
    ),
    transcript_permission: choice(
      'not_requested',
      'private_review',
      'intended_public_record'
    ),
    correction_note: text(4000)
  }
};
export const LOCKED_RESTRICTED = [
...
export const FIELD_CATALOGUE: Record<ModuleId, FieldDefinition[]> =
  Object.fromEntries(
    MODULE_IDS.map((moduleId) => [
      moduleId,
      Object.entries(schemas[moduleId]).map(([id, value_schema]) => {
        ...
        return {
          ...
          default_visibility: locked ? 'restricted' : 'public_record',
          locked_restricted: locked
        };
      })
    ])
  ) as Record<ModuleId, FieldDefinition[]>;

const PHOTOGRAPHY_REQUIRED = [
  ...
  'preservation.display_orientation_crop',
  'preservation.color_and_tone',
  'preservation.acceptable_changes'
];
export const CONFIRMATION_COPY_VERSION =
...
export const DOCUMENTATION_LIMITS = {
  ...
  assets_per_context: 100,
  upload_sessions_per_context: 5,
  upload_part_bytes: 16777216,
  client_parallel_parts: 3,
  upload_session_seconds: 86400,
...
export const PROFILES: DocumentationProfile[] = [
  ...
  profile_id,
  version: 1,
  program_id: profile_id === 'keys_and_gates_v1' ? '6529NM-AP-01' : null,
  wave_id:
    profile_id === 'keys_and_gates_v1'
      ? '4ff022b3-aa17-4a0a-ba78-58f64ff1d427'
      : null,
  required_for_review:
...
export function applyOperations(
  ...
  for (const operation of operations) {
    ...
    if (operation.op === 'unset') {
      ...
      result[operation.field] = validateAnswer(
        moduleId,
        definition,
        operation.answer
      );
...
function validateAnswer(
  ...
  if (
    ...
    !['public_record', 'restricted'].includes(answer.intended_visibility)
  )
    fail(422, 'INVALID_ANSWER');
  if (
  ...
  if (
    ...
    !matchesSchema(answer.explanation, text(1000))
  )
    fail(422, 'INVALID_ANSWER');
  if (answer.status !== 'provided') {
    if (
      ...
      (moduleId === 'process' &&
        definition.id === 'capture_method' &&
        !answer.explanation)
    )
      fail(422, 'INVALID_ANSWER');
    return answer;
  }
  if (!matchesSchema(answer.value, definition.value_schema))
...
function validateKindDetails(path: string, item: Record<string, Json>): void {
  ...
  if (
    ...
    (path === 'process.ai_use' &&
      ['assistive', 'generative'].includes(kind as string) &&
      !item.detail) ||
    (path === 'rights.rights_basis' && kind !== 'artist_owned' && !item.detail)
  )
  ...
  if (path === 'process.techniques') {
    ...
    if (
      ...
      (kinds.includes('other') && !item.other_detail)
    )
      fail(422, 'DETAIL_REQUIRED');
  }
  if (
...
function validateAvailabilityDetails(
  ...
  if (
    ...
    !(item.references as Json[] | undefined)?.length &&
    !item.explanation
  )
    fail(422, 'EXPLANATION_REQUIRED');
}
...
function validateEntries(path: string, item: Record<string, Json>): void {
  ...
  if (item.kind === 'entries_supplied' ? !entries?.length : !!entries?.length)
    fail(422, 'INVALID_ENTRIES');
  if (
    path === 'process.ingredients' &&
  ...
  if (path === 'context.history')
    for (const entry of entries ?? [])
      if (
        entry.date &&
        !validateDateObject(entry.date as Record<string, unknown>)
      )
        fail(422, 'INVALID_DATE');
}
function validateLocalized(item: Record<string, Json>): void {
  ...
  if (
    !languages.includes(item.primary_language) ||
    new Set(languages).size !== languages.length ||
    versions.filter((version) => version.authorship === 'original').length !== 1
  )
    fail(422, 'INVALID_TRANSLATIONS');
}
...
export function conditionalRequired(
  ...
  if (techniques.includes('composite') || techniques.includes('collage'))
    paths.push('process.ingredients');
  ...
  if (['includes_minors', 'uncertain'].includes(people ?? ''))
    paths.push('rights.identifiability_note');
  if (answerValue(modules.rights.consent_status) === 'documents_supplied')
    paths.push('rights.consent_asset_ids');
  return paths;
}
