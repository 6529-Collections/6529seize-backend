...
export async function calculateConsolidatedMemesTdh(
  seasons: MemesSeason[],
  tdh: ConsolidatedTDH[]
): Promise<ConsolidatedTDHMemes[]> {
  const tdhWithMemes = tdh.filter((t) => t.memes_balance > 0);
  logger.info(
    `[FOUND ${tdhWithMemes.length} WALLETS WITH MEMES] : [TOTAL WALLETS ${tdh.length}] : [SEASONS ${seasons.length}]`
  );

  const memesTdh: ConsolidatedTDHMemes[] = [];

  tdhWithMemes.forEach((t) => {
    seasons.forEach((s) => {
      const seasonMemes = t.memes.filter(
        (m) => s.start_index <= m.id && m.id <= s.end_index
      );
      const memeTotals = seasonMemes.reduce(
        (acc, cp) => {
          acc.balance += cp.balance;
      ...
      const tdhMemes: ConsolidatedTDHMemes = {
        consolidation_key: t.consolidation_key,
        season: s.id,
        balance: memeTotals.balance,
        tdh: memeTotals.tdh,
