    ...
    try {
      ...
      return await this.tdhGrantsRepository.executeNativeQueriesInTransaction(
        async (connection) => {
          ...
          if (
            [TdhGrantStatus.FAILED, TdhGrantStatus.DISABLED].includes(status)
          ) {
            throw new BadRequestException(
              `Grant ${grantId} is in status ${status}. Only grants with following statuses can be updated: ${[TdhGrantStatus.PENDING, TdhGrantStatus.GRANTED].join(`, `)}.`
            );
          }
