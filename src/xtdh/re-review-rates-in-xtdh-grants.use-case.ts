...
export class ReReviewRatesInXTdhGrantsUseCase {
  ...
  public async handle(ctx: RequestContext) {
    try {
      ...
      this.logger.info(
        `Starting to review xTDH grants rates and adjusting them in case of overflows`
      );
  ...
  private buildReplacementGrants(rows: GrantWithCap[]): XTdhGrantEntity[] {
    if (!rows.length) return [];

    const nowMillis = Time.currentMillis();
    const byGrantor = this.groupByGrantor(rows);
    const out: XTdhGrantEntity[] = [];
  ...
  private toSpans(grants: GrantWithCap[]): Span[] {
    ...
    for (const g of grants) {
      const from = Math.max(
        g.valid_from!,
        ReReviewRatesInXTdhGrantsUseCase.WINDOW_START
      );
      const toWasNull = g.valid_to == null;
      const to = toWasNull
        ? ReReviewRatesInXTdhGrantsUseCase.WINDOW_END
        : Math.min(g.valid_to, ReReviewRatesInXTdhGrantsUseCase.WINDOW_END);
  ...
  private makeReplacementEntity(
    ...
    const isWindowEnd = segEnd === ReReviewRatesInXTdhGrantsUseCase.WINDOW_END;
  ...
  private buildScaledSegmentsForGrantor(
    ...
    const segments: XTdhGrantEntity[] = [];
  ...
  private mergeAdjacentSegments(
    segments: XTdhGrantEntity[]
  ): XTdhGrantEntity[] {
    const merged: XTdhGrantEntity[] = [];
    const WINDOW_END = ReReviewRatesInXTdhGrantsUseCase.WINDOW_END;
