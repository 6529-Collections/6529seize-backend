...
const PREVIOUS_PRODUCER_BACKEND_WORKFLOW_BLOB =
...
const PREVIOUS_PRODUCER_POLICY_DIGEST_BEFORE_CONCURRENCY =
...
describe('Release Bus PR CI producer bridge', () => {
  it('preauthorizes only the frozen backend and frontend producer workflows', () => {
    for (const expected of [
      ...
      PREVIOUS_PRODUCER_BACKEND_WORKFLOW_BLOB,
  ...
  it('binds the exact bridge and producer policy bundle digests', () => {
    for (const expected of [
      ...
      PREVIOUS_PRODUCER_POLICY_DIGEST_BEFORE_CONCURRENCY,
