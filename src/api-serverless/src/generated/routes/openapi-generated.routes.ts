...
import { handlePreflightMarketBatch } from '@/api/marketplace/marketplace-batch-preflight.handler';
...
import { handleRefreshPushInstallationBadge } from '@/api/push-notifications/refresh-push-installation-badge.handler';
...
import { handleDeleteMyWaveChatHistory, handlePrepareMyWaveChatHistoryPurge } from '@/api/waves/delete-my-wave-chat-history.handler';
...
router.post(
  '/drop-media/proposal-frame',
  needsAuthenticatedUser(),
  async (
    req: CreateProposalFrameRequest,
    res: Response<ApiResponse<CreateProposalFrameResponse>>
  ) => {
    res.send(await handleCreateProposalFrame(req));
  }
);
...
router.post(
  '/market/operations/:id/preflight',
  needsAuthenticatedUser(),
  async (
    req: PreflightMarketBatchRequest,
    res: Response<ApiResponse<PreflightMarketBatchResponse>>
  ) => {
    res.send(await handlePreflightMarketBatch(req));
  }
);
...
router.post(
  '/push-notifications/installations/badge-refresh',
  async (
    req: RefreshPushInstallationBadgeRequest,
    res: Response<ApiResponse<RefreshPushInstallationBadgeResponse>>
  ) => {
    res.send(await handleRefreshPushInstallationBadge(req));
  }
);
...
router.post(
  ...
  async (
    ...
    res: Response<ApiResponse<PrepareMyWaveChatHistoryPurgeResponse>>
  ) => {
    res.send(await handlePrepareMyWaveChatHistoryPurge(req));
  }
);
