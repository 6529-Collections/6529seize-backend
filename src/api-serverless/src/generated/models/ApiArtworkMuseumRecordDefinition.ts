/**
 * 6529.io API
 * JSON REST API for 6529.io. New here? A terminology overview and a step-by-step authentication walkthrough (guides &amp; auth) live at <a href=\"https://6529.io/tools/api\">https://6529.io/tools/api</a>. The raw machine-readable spec is downloadable at <a href=\"/openapi.yaml\">/openapi.yaml</a> and <a href=\"/openapi.json\">/openapi.json</a>.
 *
 * OpenAPI spec version: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { ApiArtworkDocumentationValueSchema } from '../models/ApiArtworkDocumentationValueSchema';
import { HttpFile } from '../http/http';

export class ApiArtworkMuseumRecordDefinition {
    'kind': string;
    'label': string;
    'description': string;
    'lane': ApiArtworkMuseumRecordDefinitionLaneEnum;
    'stream_schema': string;
    'value_schema': ApiArtworkDocumentationValueSchema;

    static readonly discriminator: string | undefined = undefined;

    static readonly mapping: {[index: string]: string} | undefined = undefined;

    static readonly attributeTypeMap: Array<{name: string, baseName: string, type: string, format: string}> = [
        {
            "name": "kind",
            "baseName": "kind",
            "type": "string",
            "format": ""
        },
        {
            "name": "label",
            "baseName": "label",
            "type": "string",
            "format": ""
        },
        {
            "name": "description",
            "baseName": "description",
            "type": "string",
            "format": ""
        },
        {
            "name": "lane",
            "baseName": "lane",
            "type": "ApiArtworkMuseumRecordDefinitionLaneEnum",
            "format": ""
        },
        {
            "name": "stream_schema",
            "baseName": "stream_schema",
            "type": "string",
            "format": ""
        },
        {
            "name": "value_schema",
            "baseName": "value_schema",
            "type": "ApiArtworkDocumentationValueSchema",
            "format": ""
        }    ];

    static getAttributeTypeMap() {
        return ApiArtworkMuseumRecordDefinition.attributeTypeMap;
    }

    public constructor() {
    }
}

export enum ApiArtworkMuseumRecordDefinitionLaneEnum {
    Curatorial = 'curatorial',
    Technical = 'technical',
    Rights = 'rights'
}

