/**
 * 6529.io API
 * JSON REST API for 6529.io. New here? A terminology overview and a step-by-step authentication walkthrough (guides &amp; auth) live at <a href=\"https://6529.io/tools/api\">https://6529.io/tools/api</a>. The raw machine-readable spec is downloadable at <a href=\"/openapi.yaml\">/openapi.yaml</a> and <a href=\"/openapi.json\">/openapi.json</a>.
 *
 * OpenAPI spec version: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { ApiMarketBatchSettlementItem } from '../models/ApiMarketBatchSettlementItem';
import { HttpFile } from '../http/http';

export class ApiMarketBatchSettlement {
    'outcome': ApiMarketBatchSettlementOutcomeEnum;
    'items': Array<ApiMarketBatchSettlementItem>;
    'transaction_hash': string;
    'block_number': number;
    'block_hash': string;
    'safe_block_number'?: number;

    static readonly discriminator: string | undefined = undefined;

    static readonly mapping: {[index: string]: string} | undefined = undefined;

    static readonly attributeTypeMap: Array<{name: string, baseName: string, type: string, format: string}> = [
        {
            "name": "outcome",
            "baseName": "outcome",
            "type": "ApiMarketBatchSettlementOutcomeEnum",
            "format": ""
        },
        {
            "name": "items",
            "baseName": "items",
            "type": "Array<ApiMarketBatchSettlementItem>",
            "format": ""
        },
        {
            "name": "transaction_hash",
            "baseName": "transaction_hash",
            "type": "string",
            "format": ""
        },
        {
            "name": "block_number",
            "baseName": "block_number",
            "type": "number",
            "format": "int64"
        },
        {
            "name": "block_hash",
            "baseName": "block_hash",
            "type": "string",
            "format": ""
        },
        {
            "name": "safe_block_number",
            "baseName": "safe_block_number",
            "type": "number",
            "format": "int64"
        }    ];

    static getAttributeTypeMap() {
        return ApiMarketBatchSettlement.attributeTypeMap;
    }

    public constructor() {
    }
}

export enum ApiMarketBatchSettlementOutcomeEnum {
    AllSelected = 'ALL_SELECTED'
}

