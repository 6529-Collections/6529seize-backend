...
async function updateSubscriptionsAfterSubscribeAllEditionsChange(
  ...
  const subscriptionEligibility =
    await fetchSubscriptionEligibility(consolidation_key);
  ...
  upcomingSubscriptions.forEach((subscription) => {
    const log = `Updated edition preference for Meme #${subscription.token_id}`;
    const additionalInfo = `Edition Preference: ${subscribe_all_editions ? 'All eligible' : 'One edition'} - Eligibility: x${subscriptionEligibility}`;
    promises.push(
      sqlExecutor.execute(
        `
        INSERT INTO ${SUBSCRIPTIONS_NFTS_TABLE} (consolidation_key, contract, token_id, subscribed_count)
        VALUES (:consolidation_key, :contract, :tokenId, :subscribed_count)
    ...
    promises.push(
      sqlExecutor.execute(
        ...
        VALUES (:consolidation_key, :log, :additionalInfo)
        `,
        {
          consolidation_key,
          log,
