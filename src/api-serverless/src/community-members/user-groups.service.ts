...
const ELIGIBLE_GROUPS_LEASE_RENEW_MS = 1_000;
...
async function resolveEligibilityReads<T extends readonly unknown[]>(reads: {
  readonly [K in keyof T]: () => Promise<T[K]> | T[K];
}): Promise<T> {
  return Promise.all(reads.map((read) => read())) as unknown as Promise<T>;
}
...
export class UserGroupsService {
  ...
  private async getGivenGroupEntities(
    ...
    return this.timeAsync(
      ...
      () => this.userGroupsDb.getByIds(givenGroups, { timer })
    );
  }
  ...
  private async getGroupsUserIsEligibleForWithCache(
    ...
    while (!signal?.aborted && performance.now() < deadline) {
      ...
      waited = true;
      const peerEntry = await this.readEligibleGroupsResultAfterWait(
        profileId,
        ttlSec,
        timer,
        signal
      ...
      if (remaining > 0) {
        await Time.millis(Math.min(remaining, 125 + randomInt(176))).sleep();
      }
    }
    this.eligibilityUnavailable('timeout');
  }

  private async getInitiallyCachedEligibleGroups(
    profileId: string,
    ttlSec: number,
    timer?: Timer
  ): Promise<string[] | null> {
  ...
  private async readEligibleGroupsResultAfterWait(
    profileId: string,
    ttlSec: number,
    timer?: Timer,
    signal?: AbortSignal
