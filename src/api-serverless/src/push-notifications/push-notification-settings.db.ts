...
export class PushNotificationSettingsDb extends LazyDbAccessCompatibleService {
  public async getPushNotificationSettings(
    profileId: string,
    deviceId: string
  ): Promise<PushNotificationSettingsData> {
    const result = await this.db.oneOrNull<PushNotificationSettingsEntity>(
      `
  ...
  public async upsertPushNotificationSettings(
    profileId: string,
    deviceId: string,
    settings: Partial<PushNotificationSettingsData>
  ): Promise<PushNotificationSettingsData> {
    const currentSettings = await this.getPushNotificationSettings(
      profileId,
      deviceId
    );
    ...
    await this.db.execute(
      `
        ...
        ON DUPLICATE KEY UPDATE
          ...
          identity_nic = VALUES(identity_nic),
...
export const pushNotificationSettingsDb = new PushNotificationSettingsDb(
  dbSupplier
);
