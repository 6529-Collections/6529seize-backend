...
export class GitHubDeployService {
  ...
  public async dispatchDeploy(params: {
    ...
    releasePullRequest?: number | null;
    releaseGroupServices?: string;
    releaseNoteOptOut?: boolean;
  }): Promise<void> {
    const target = params.target ?? 'backend';
    const inputs: Record<string, string> = {
      release_note_opt_out: String(params.releaseNoteOptOut ?? false)
    };
    if (target === 'backend') {
      inputs.environment = params.environment;
      inputs.service = params.service ?? '';
      if (params.environment === 'prod' && !params.releaseNoteOptOut) {
        inputs.release_pull_request = params.releasePullRequest
          ? String(params.releasePullRequest)
          : '';
        inputs.release_group_services =
          params.releaseGroupServices || params.service || '';
        inputs.release_note_publish = 'true';
      }
    }
    const body = { ref: params.ref, inputs };
    const response = await this.api(
      params.token,
      target,
