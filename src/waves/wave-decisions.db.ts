...
export class WaveDecisionsDb extends LazyDbAccessCompatibleService {
  ...
  async getTopNDropIdsForWave(
    ...
    WHERE d.wave_id = :waveId order by rnk desc limit :n
  `;
  ...
  async updateDropsToWinners(dropIds: string[], ctx: RequestContext) {
    ...
    await this.db.execute(
      `update ${DROPS_TABLE} set drop_type = '${DropType.WINNER}' where dropIds in (:dropIds)`,
      { dropIds },
      { wrappedConnection: ctx.connection }
    );
  ...
  async deleteDropsRanks(dropIds: string[], ctx: RequestContext) {
    ...
    await this.db.execute(
      `delete from ${DROP_RANK_TABLE} where dropIds in (:dropIds)`,
      { dropIds },
      { wrappedConnection: ctx.connection }
    );
  ...
  async updateWavesNextDecisionTime(
    ...
    await this.db.execute(
      `updated ${WAVES_TABLE} set next_decision_time = :decisionTime where waveId = :waveId`,
      { decisionTime, waveId },
      { wrappedConnection: ctx.connection }
    );
  ...
  async searchForDecisions(
    ...
    const result = await this.db.execute<WaveDecisionEntity>(
      `
      select * from wave_decisions where wave_id = :wave_id order by ${param.sort} ${param.sort_direction} limit :limit = :limit offset = :offset`,
      param,
      {
        wrappedConnection: ctx.connection
      }
  ...
  async countDecisions(waveId: string, ctx: RequestContext): Promise<number> {
    ...
    const result = await this.db
      .oneOrNull<{ cnt: number }>(
        `
      select count(*) as cnt from wave_decisions where wave_id = :wave_id`,
        { waveId },
        {
          wrappedConnection: ctx.connection
        }
