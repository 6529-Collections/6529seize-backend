...
| `releaseNotesGenerationLoop`     | SQS `release-note-generation`                                                                                                      | Turn successful production deploy ranges into repository-prompted release notes and publish them as `ci6529`.              |
...
Production CI notifications also feed the release-note queue. The API first posts the normal CI status drop. For a successful production notification that carries an allowlisted repository prompt path, it then sends the repository, workflow run, deployed SHA, release-group id and services, deployment time, and prompt path to `release-note-generation`. A frontend deploy is a one-service group. The backend `ghdeploy` batch path assigns one group id and the complete selected-service list to every service workflow; the worker records each successful service in Redis and generates exactly one note only after the full group succeeds. `releaseNotesGenerationLoop` loads the reviewed prompt from the deployed repository SHA through GitHub, finds the previous matching successful production workflow run while excluding other runs at the current grouped SHA, associates commits in the deployed range with merged pull requests, calls OpenAI, resolves configured GitHub contributors to 6529 profile mentions, and posts one line per pull request plus the grouped backend service list to `CI_RELEASES_WAVE_ID` as the profile configured by `CI_PIPELINES_BOT_PROFILE_ID`. Redis is required and keys scoped by repository, release group, and deployed SHA suppress redelivery duplicates; the group and dedupe state are retained for 90 days, and the SQS DLQ retains repeated processing failures.
