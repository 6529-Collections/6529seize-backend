...
| `releaseNotesGenerationLoop`     | SQS `release-note-generation`                                                                                                      | Publish production Backend, Frontend, and Desktop release notes as `ci6529`, then attach Frontend post-deployment validation replies. |
...
Production CI notifications also feed the release-note queue; staging notifications never carry release-note fields. The HMAC-signed `/ci-pipeline-alerts` automation ingress is mounted manually and intentionally excluded from the public OpenAPI surface. Required Frontend release-note queue writes complete before the normal CI status drop is created, so a failed enqueue can be retried without duplicating that alert; Backend and Desktop queue writes retain their best-effort behavior. For a successful production notification with an allowlisted repository prompt path, the API accepts either the legacy single release group or a v2 array of PR-scoped groups and enqueues one message per group. Each group contains its merged PR number and complete canonical service set, so one service deployment may update multiple overlapping PR groups. Frontend and Desktop publishes are one-service groups. Each successful backend service deploy records its workflow run under the merged PR number; runs may use different descendant SHAs. Every applicable successful service persists the group-level publish request, so no particular service or completion order owns finalization. The canonical completed-service set gates generation, and the Redis processing lock is the sole concurrent publication winner.

`releaseNotesGenerationLoop` loads the reviewed prompt from the deployed repository SHA through GitHub, finds the previous matching successful production workflow run while excluding other runs at the current grouped SHA, and calls Amazon Bedrock using `RELEASE_NOTES_BEDROCK_MODEL_ID` or the Claude Sonnet 4.5 US geo inference profile by default. A Bedrock system instruction treats the tagged release context as untrusted data and rejects instructions embedded in pull requests, commits, or filenames. Backend and Frontend notes associate commits with merged pull requests, resolve configured GitHub contributors to 6529 profile mentions, and render deterministic repository-specific headings and service links. Desktop notes are requested only by the production `Publish` flow after its S3 download pages exist. They compare against the previous successful production `Publish` across version branches, retain the Core first-parent change history so imported Frontend history is not summarized twice, and resolve the exact Frontend release-note drop from the renderer-source SHA. New release drops carry repository, SHA, run, and deployment metadata; historical Frontend notes are matched only when their content contains the exact full commit URL. Desktop output has a deterministic heading, Frontend release link, and platform download links around compact model-generated user-facing bullets, with no PR or contributor lines.

All notes publish to `CI_RELEASES_WAVE_ID` as the profile configured by `CI_PIPELINES_BOT_PROFILE_ID`. Each published drop carries a deterministic release-note metadata id; the worker checks it before generation so a crash after drop creation but before the Redis completion write cannot publish the same release twice. Redis is required; PR-scoped group, publish-request, and dedupe state is retained for 90 days. A failed Desktop queue handoff immediately posts a normal production CI failure with the shared 🚨 heading and `@devs6529` mention. Once queued, a Desktop request is attempted once and retried by SQS up to three times. If the fourth attempt still fails, the worker posts the same class of production CI failure without blocking the remaining Desktop publish steps. If delivery of that terminal alert fails, the fifth receive fails closed without posting a duplicate alert and moves the request to the SQS DLQ.

Frontend production validation notifications use the same queue and release-group-plus-SHA identity to locate the release-note drop and publish a threaded success or failure reply. Backend, Desktop, and PR-scoped validation identities are rejected because those deployments do not emit this E2E lifecycle. Failures tag `devs6529`; automatic failures name the deployment initiator and manual failures name the revalidation initiator. Known GitHub-to-6529 mappings create a profile mention, while unmapped logins remain linked to GitHub. Automatic and manual revalidation runs have distinct deterministic identities, so a later successful manual recovery appears chronologically beneath the earlier failure. If validation finishes before its release note is visible, the worker fails the SQS message so normal retry resolves the race. Deterministic validation metadata prevents duplicate replies after redelivery.

Deploy the release-validation pieces in dependency order:

1. `releaseNotesGenerationLoop`
2. `api`
3. `releaseBus`

Release Bus E2E dispatch remains compatible with the simplified Frontend workflows without restoring the former Frontend control plane. Legacy workflow revisions receive legacy manifest inputs; simplified revisions receive only the exact deployed Frontend SHA and an opaque tracking ID. Before binding a discovered run, the backend verifies the GitHub App actor, workflow-dispatch event, exact workflow path, immutable control SHA, and operation tracking identity.
