...
flowchart TD
  ...
  subgraph Scheduled["Scheduled Lambdas (EventBridge)"]
    ...
    WaveScoreRefreshLoop ~~~ MembershipRefreshLoop["membershipRefreshLoop"]
    MembershipRefreshLoop ~~~ XTdhGrantsReviewerLoop["xTdhGrantsReviewerLoop"]
  ...
  subgraph Queues["Queue and topic triggered Lambdas"]
    ...
    SeizeAPI --> MembershipDirtyRows["MySQL: membership_refresh_requests"]
    NftOwnersLoop --> MembershipDirtyRows
    DelegationsLoop --> MembershipDirtyRows
    ExternalLiveTailLoop --> MembershipDirtyRows
    XTdhLoop --> MembershipDirtyRows
    MembershipDirtyRows --> MembershipDirtyQueue["SQS: membership-refresh-dirty.fifo"] --> MembershipRefreshLoop
    MembershipRefreshLoop --> MembershipStartQueue["SQS: membership-refresh-start.fifo"]
    MembershipStartQueue --> MembershipRefreshLoop
    MembershipRefreshLoop --> MaterializedMembership["MySQL: user_group_members<br/>states + watermarks"]
    MaterializedMembership --> SeizeAPI
...
| `membershipRefreshLoop`              | Drain eligibility refresh requests and run resumable full backfills. |
...
| `membershipRefreshLoop`          | SQS `membership-refresh-start.fifo` and `membership-refresh-dirty.fifo`; nightly/full and one-minute EventBridge schedules         | Materialize profile-to-wave-group eligibility, drain durable dirty targets, and advance full/grant-boundary watermarks.                    |
...
Eligibility materialization uses a fail-closed DB-backed/SQS pipeline. Ratings, explicit profile-group edits, group/wave changes, NFT ownership, wallet consolidations, xTDH, xTDH grant status, grant time boundaries, and external collection transfers write coalesced `membership_refresh_requests` before or in the same transaction as the source mutation. `membershipRefreshLoop` drains those requests into `user_group_members`, records per-target `membership_materialization_states`, and uses `membership_watermarks` for the spec-versioned full backfill, grant-boundary sweep, and missing-profile cursor. The missing-profile sweep reads at most 1,000 identities per invocation in primary-key order and checks state only for that bounded page, avoiding a recurring full-table anti-join. The Lambda is runtime-isolated but its function, queues, schedules, and alarms are owned by the allowlisted `dbMigrationsLoop` CloudFormation stack, so a single deploy unit establishes the additive schema and refresh consumer before producer hooks are deployed. A selected `(scope, target_id, dirty_at)` request is deleted only if that exact version still exists, so a target dirtied during calculation remains queued. A target that fails five times remains as a fail-closed poison row and raises a CloudWatch alarm; a later source mutation resets its retry budget. Both full and dirty FIFO messages redrive to the alarmed DLQ after repeated handler-level failures.

The API reads `user_group_members` from the primary DB only when the current-spec full watermark exists, the profile has a current materialization state, and there are no dirty requests for that profile or any group. Otherwise it immediately runs the legacy eligibility computation, so deployment, failed jobs, new profiles, full refreshes, and producer lag cannot turn stale materialization into an allow decision. The default and `ELIGIBILITY_READ_MODE=legacy` disable the path, `shadow` compares it against legacy results, and `materialized` serves ready rows with the same fallback. Starting a full backfill removes its readiness watermark and snapshots the profile cohort; finalization prunes non-wave rows, captures grant boundaries crossed during the run, and restores the watermark atomically. Profiles created during or after the run stay fail-closed until the bounded missing-profile cursor marks them dirty. A one-minute EventBridge run covers missed SQS wakeups, while the nightly full pass repairs any producer gap.
