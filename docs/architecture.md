...
- Desktop Core support uses the same frontend index before public-data planning: local-node questions preserve complete `desktop-core` procedures with a larger bounded reply budget. The bot cannot inspect local state or perform repairs; generic website and mobile questions retain normal routing. See `specs/6529-help-bot-runtime.md` for the knowledge and fallback contract.
