...
Production CI notifications also feed the release-note queue. The API first posts the normal CI status drop. For a successful production notification that carries a repository-owned release-note prompt, it then sends the repository, workflow run, deployed SHA, release-group id and services, deployment time, and prompt to `release-note-generation`. A frontend deploy is a one-service group. The backend `ghdeploy` batch path assigns one group id and the complete selected-service list to every service workflow; the worker records each successful service in Redis and generates exactly one note only after the full group succeeds. `releaseNotesGenerationLoop` finds the previous matching successful production workflow run through GitHub while excluding other runs at the current grouped SHA, associates commits in the deployed range with merged pull requests, calls OpenAI with the reviewed repository prompt, resolves configured GitHub contributors to 6529 profile mentions, and posts one line per pull request plus the grouped backend service list to `CI_RELEASES_WAVE_ID` as the profile configured by `CI_PIPELINES_BOT_PROFILE_ID`. Redis keys scoped by repository, release group, and deployed SHA suppress redelivery duplicates; the SQS DLQ retains repeated processing failures.
