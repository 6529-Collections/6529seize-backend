...
Production CI notifications also feed the release-note queue; staging notifications never carry release-note fields. The API first posts the normal CI status drop. For a successful production notification with an allowlisted repository prompt path, it accepts either the legacy single release group or a v2 array of PR-scoped groups and enqueues one message per group. Frontend release-note and validation queue writes are required and propagate failure to the signed caller; backend multi-service release-note writes retain their existing best-effort behavior. Each group contains its merged PR number and complete canonical service set, so one service deployment may update multiple overlapping PR groups. A frontend deploy is a one-service group. Each successful backend service deploy records its workflow run under the merged PR number; runs may use different descendant SHAs. Every applicable successful service persists the group-level publish request, so no particular service or completion order owns finalization. The canonical completed-service set gates generation, and the Redis processing lock is the sole concurrent publication winner. `releaseNotesGenerationLoop` loads the reviewed prompt from the deployed repository SHA through GitHub, finds the previous matching successful production workflow run while excluding other runs at the current grouped SHA, associates commits in the deployed range with merged pull requests, calls Amazon Bedrock using `RELEASE_NOTES_BEDROCK_MODEL_ID` or the Claude Sonnet 4.5 US geo inference profile by default, resolves configured GitHub contributors to 6529 profile mentions, and posts one line per pull request with deterministic service labels for backend PRs to `CI_RELEASES_WAVE_ID` as the profile configured by `CI_PIPELINES_BOT_PROFILE_ID`. When no baseline or merged pull request exists, it publishes a minimal deployment record with the same deterministic metadata so validation always has a parent. Single-service headings link their workflow run; grouped backend notes list the run for every deployed service and omit that optional line if the run metadata is incomplete. Each published drop carries a deterministic release-note metadata id; the worker checks it before generation so a crash after drop creation but before the Redis completion write cannot publish the same release twice. A later production validation notification uses the same queue and release-group identity to locate that drop and post a threaded success or failure reply; failures tag `devs6529`. Automatic and manual revalidation runs have distinct deterministic identities, preserving chronological failure and recovery replies in the same thread. If validation finishes before the release note is visible, the worker fails the message so normal SQS retry resolves the race. Deterministic validation metadata prevents duplicate replies after redelivery. Redis is required; PR-scoped group, publish-request, and dedupe state is retained for 90 days, and the SQS DLQ retains repeated processing failures.

Release Bus E2E dispatch remains compatible with the simplified frontend workflows without restoring the former frontend control plane. Before dispatch, the backend reads the immutable frontend workflow contract. Legacy workflow revisions receive the legacy manifest inputs and continue reporting structured callbacks. Simplified revisions receive only the exact deployed frontend SHA and an opaque tracking id; the frontend admits that dispatch mode only for the Release Bus GitHub App, independently verifies the live deployed SHA, and runs under the environment deployment lock. The backend binds the run to the exact immutable workflow revision and accepts its terminal GitHub conclusion as E2E evidence.
