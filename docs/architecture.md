...
| `releaseNotesGenerationLoop`     | SQS `release-note-generation`                                                                                                      | Production only: publish repository-prompted release notes as `ci6529`, then attach post-deployment validation replies.     |
...
Production CI notifications also feed the release-note queue; staging notifications never carry release-note fields. The HMAC-signed `/ci-pipeline-alerts` automation ingress is mounted manually and intentionally excluded from the public OpenAPI surface. For a successful production notification with an allowlisted repository prompt path, the API accepts either the legacy single release group or a v2 array of PR-scoped groups and enqueues one message per group. Required frontend release-note queue writes complete before the normal CI status drop is created, so a failed enqueue can be retried without duplicating that alert; backend multi-service release-note writes retain their existing best-effort behavior. Each group contains its merged PR number and complete canonical service set, so one service deployment may update multiple overlapping PR groups. A frontend deploy is a one-service group and always uses its release-group-plus-SHA identity without a pull-request number. Each successful backend service deploy records its workflow run under the merged PR number; runs may use different descendant SHAs. Every applicable successful service persists the group-level publish request, so no particular service or completion order owns finalization. The canonical completed-service set gates generation, and the Redis processing lock is the sole concurrent publication winner. `releaseNotesGenerationLoop` loads the reviewed prompt from the deployed repository SHA through GitHub, finds the previous matching successful production workflow run while excluding other runs at the current grouped SHA, associates commits in the deployed range with merged pull requests, calls Amazon Bedrock using `RELEASE_NOTES_BEDROCK_MODEL_ID` or the Claude Sonnet 4.5 US geo inference profile by default, resolves configured GitHub contributors to 6529 profile mentions, and posts one line per pull request with deterministic service labels for backend PRs to `CI_RELEASES_WAVE_ID` as the profile configured by `CI_PIPELINES_BOT_PROFILE_ID`. When no baseline or merged pull request exists, it publishes a minimal deployment record with the same deterministic metadata so validation always has a parent. Single-service headings link their workflow run; grouped backend notes list the run for every deployed service and omit that optional line if the run metadata is incomplete. Each published drop carries a deterministic release-note metadata id; the worker checks it before generation so a crash after drop creation but before the Redis completion write cannot publish the same release twice. A later frontend production validation notification uses the same queue and release-group-plus-SHA identity to locate that drop and post a threaded success or failure reply; backend and PR-scoped validation identities are rejected because backend deployments do not emit this E2E lifecycle. Failures tag `devs6529`. Automatic and manual revalidation runs have distinct deterministic identities, preserving chronological failure and recovery replies in the same thread. If validation finishes before the release note is visible, the worker logs the missing publication id and fails the message so normal SQS retry resolves the race. Deterministic validation metadata prevents duplicate replies after redelivery. Redis is required; PR-scoped group, publish-request, and dedupe state is retained for 90 days, and the SQS DLQ retains repeated processing failures.

Failed frontend validation replies also identify the responsible human. Automatic validation names the deployment initiator, while manual validation names the revalidation initiator. Known GitHub-to-6529 mappings produce a real profile mention; unmapped logins remain linked to their GitHub profile. Both failure forms continue to tag `devs6529`.

Deploy this release-validation change in dependency order:

1. `releaseNotesGenerationLoop`
2. `api`
3. `releaseBus`

Release Bus E2E dispatch remains compatible with the simplified frontend workflows without restoring the former frontend control plane. Before dispatch, the backend reads the immutable frontend workflow contract. Legacy workflow revisions receive the legacy manifest inputs and continue reporting structured callbacks. Simplified revisions receive only the exact deployed frontend SHA and an opaque tracking id; the frontend admits that dispatch mode only for the Release Bus GitHub App, independently verifies the live deployed SHA, and runs under the environment deployment lock. Before binding any discovered run, the backend filters same-title candidates by the GitHub App actor, workflow-dispatch event, exact workflow path, immutable control SHA, and operation tracking identity, then accepts the matching run's terminal GitHub conclusion as E2E evidence.
