...
- Bedrock rendering uses a configurable short timeout and a shared Anthropic Claude model default, with per-service env overrides; if rendering fails or times out, the worker falls back to deterministic wording when a reliable frontend record or public DB row exists.
