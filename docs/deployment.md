...
cleanup requires no migration or `dbMigrationsLoop` deployment. Staging also
retains its existing `membership-backfill-probes` scope and three probe indexes;
these are schema compatibility metadata only, with no backfill runtime consumer.
...
See [CI Pipeline Alerts](ci-pipeline-alerts.md) and the [deployment skill](../ops/skills/deploy-6529/SKILL.md).
