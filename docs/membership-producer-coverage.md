...
| Profile primary address and identity consolidation: `delegationsLoop/index.ts`, `identity.ts`, `db.ts`, `profiles.service.ts` | delegationsLoop, tdhLoop | GLOBAL IDENTITY, TDH_XTDH, RATINGS, GRANTS, DELEGATIONS, and OWNERSHIP when a consolidation event or old row in the inclusive replay range exists → FULL | Read the chain range before starting a new cycle, then persist that same response. Cycles that cannot change consolidation rows use `delegation-no-ownership` and leave OWNERSHIP free for the NFT owner loop. Eventful and previously started `delegation` cycles retain all six keys. One shared GLOBAL barrier stays active through xTDH universe and stats activation. The block marker commits with `TDH_INPUTS_COMMITTED`. A prior delegation cycle still at `STARTED` requires operator rebuild and repair because its procedural registration/revocation suffix cannot safely replay after a partial commit. |
