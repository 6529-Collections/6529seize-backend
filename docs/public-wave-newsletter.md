...
`Deploy a service` on `main`, with `environment=prod`. No migration or API/frontend
deployment is required. Its existing `lambda-vpc-role` needs Bedrock InvokeModel
...
Deploying with any required setting missing leaves the schedule harmlessly
disabled. Configure all three only when publication is intended. Error reporting
